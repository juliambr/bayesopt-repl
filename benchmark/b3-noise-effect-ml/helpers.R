...
computeFictionalReturnValue = function(par.set, run, fun) {
	...
	if (length(run$models) != 0) {
		...
		# dummy
		dummy = data.frame(replicate(getParamLengths(par.set) + 1, replicate(nrow(opdf), NA)))
		res1 = data.frame(dummy)
		names(res1) = paste("pred_best_", c(getParamIds(par.set), "y"), sep = "")		
	}
	...
	res2 = lapply(unique(opdf$dob), function(d) {
		...
		opdf_agg = setDT(opdf_red)[, mean := mean(y), by = "x"]
		ret = which.min(opdf_agg$mean)
		x = data.frame(opdf_agg)[ret, getParamIds(par.set)]
		y = opdf_agg[ret, ]$mean
		ytrue = getMeanFunction(fun)(x)
		c(x = x, y = y, ytrue = ytrue)
	})
